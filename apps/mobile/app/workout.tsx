...
import { completeWorkout, discardWorkout, getDraftWorkout, saveExerciseSet, skipExercise, type DraftWorkout, type WorkoutExercise } from '@/workouts/workout-api';

type ExerciseInput = { weightKg: string; reps: string; durationSeconds: string; skipReason: string };
...
export default function WorkoutScreen() {
  ...
  const loadDraft = useCallback(async () => {
    ...
    try {
      ...
      setWorkout(draft);
      setInputs(Object.fromEntries((draft?.exercises ?? []).map((exercise) => [exercise.id, inputFromExercise(exercise)])));
      setStatus('ready');
    } catch (err) {
  ...
  if (status === 'error') {
    return (
      <View style={styles.container}>
        ...
        <Pressable style={styles.secondaryButton} onPress={() => void loadDraft()}>
  ...
  if (!workout) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Workout</Text>
        <Text style={styles.description}>No draft workout yet. Start the next training day from Home.</Text>
      </View>
    );
  }

  const updateInput = (exerciseId: string, patch: Partial<ExerciseInput>) => {
    setInputs((current) => ({ ...current, [exerciseId]: { ...emptyInput, ...current[exerciseId], ...patch } }));
  };
  ...
  const saveExercise = async (exercise: WorkoutExercise) => {
    ...
    setStatus('saving');
    try {
      const set: { weightKg?: number; reps?: number; durationSeconds?: number } = {};
      if (input.weightKg) set.weightKg = Number(input.weightKg);
      if (input.reps) set.reps = Number(input.reps);
      if (input.durationSeconds) set.durationSeconds = Number(input.durationSeconds);
  ...
  const discard = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Unable to discard workout');
      setStatus('error');
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.description}>Started {new Date(workout.startedAt).toLocaleString()}</Text>
      {status === 'saving' ? <ActivityIndicator color={colors.primary} /> : null}

      <Checklist title="Warmup" items={workout.checklist.filter((item) => item.kind === 'warmup')} />
          ...
          <View key={exercise.id} style={styles.card}>
            ...
            <View style={styles.inputRow}>
              <TextInput
                ...
                onChangeText={(value) => updateInput(exercise.id, { weightKg: value })}
                placeholder="kg"
              ...
              <TextInput
                ...
                onChangeText={(value) => updateInput(exercise.id, { durationSeconds: value })}
                placeholder="sec"
                placeholderTextColor={colors.mutedText}
                keyboardType="number-pad"
                style={styles.input}
            ...
            <TextInput
              ...
              onChangeText={(value) => updateInput(exercise.id, { skipReason: value })}
            ...
            <View style={styles.buttonRow}>
              <Pressable style={styles.primaryButton} onPress={() => void saveExercise(exercise)}>
                <Text style={styles.primaryButtonText}>Save set</Text>
              </Pressable>
              <Pressable style={styles.secondaryButtonCompact} onPress={() => void skip(exercise)}>
      ...
      <Checklist title="Cooldown" items={workout.checklist.filter((item) => item.kind === 'cooldown')} />
...
function Checklist({ title, items }: { title: string; items: DraftWorkout['checklist'] }) {
  if (!items.length) return null;
  return (
    <View style={styles.card}>
      <Text style={styles.cardTitle}>{title}</Text>
      {items.map((item) => (
        <Text key={item.checklistItemId} style={styles.status}>• {item.text}</Text>
      ))}
    </View>
  );
}

const emptyInput: ExerciseInput = { weightKg: '', reps: '', durationSeconds: '', skipReason: '' };
...
function inputFromExercise(exercise: WorkoutExercise): ExerciseInput {
  ...
  return {
    ...
    durationSeconds: firstSet?.durationSeconds ? String(firstSet.durationSeconds) : '',
    skipReason: exercise.status === 'skipped' ? (exercise.note ?? '') : '',
  };
}

function replaceExercise(workout: DraftWorkout, exercise: WorkoutExercise): DraftWorkout {
  return { ...workout, exercises: workout.exercises.map((item) => (item.id === exercise.id ? exercise : item)) };
}

function formatSet(set: WorkoutExercise['sets'][number]) {
  if (set.durationSeconds) return `Set ${set.setIndex}: ${set.durationSeconds}s`;
...
const styles = StyleSheet.create({
  ...
  card: { gap: spacing.sm, padding: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: 16, backgroundColor: colors.card },
  cardTitle: { color: colors.text, fontSize: 18, fontWeight: '800' },
  status: { color: colors.mutedText, fontSize: 14, lineHeight: 20 },
  inputRow: { flexDirection: 'row', gap: spacing.sm },
  input: { flex: 1, minWidth: 72, padding: spacing.sm, borderWidth: 1, borderColor: colors.border, borderRadius: 12, color: colors.text, backgroundColor: colors.surface },
  fullInput: { padding: spacing.sm, borderWidth: 1, borderColor: colors.border, borderRadius: 12, color: colors.text, backgroundColor: colors.surface },
  buttonRow: { flexDirection: 'row', gap: spacing.sm },
  primaryButton: { alignItems: 'center', padding: spacing.md, borderRadius: 999, backgroundColor: colors.primary },
  ...
  secondaryButtonCompact: { alignItems: 'center', padding: spacing.md, borderRadius: 999, borderWidth: 1, borderColor: colors.border, flex: 1 },

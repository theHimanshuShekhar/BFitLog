...
export function BodyWeightChart({ logs, goal, range }: Props) {
  ...
  const points = [...visibleLogs].sort((a, b) => a.measuredAt.localeCompare(b.measuredAt));

  if (points.length === 0) {
    return (
      <View style={styles.empty}>
        <Text style={styles.emptyText}>No body weight logs in this range yet.</Text>
      </View>
    );
  }
  ...
  const scaled = points.map((log) => {
    const x = padding + ((new Date(log.measuredAt).getTime() - firstTime) / timeSpan) * (width - padding * 2);
    const y = padding + ((maxWeight - log.weightKg) / (maxWeight - minWeight)) * (height - padding * 2);
    return { x, y, log };
  });
  ...
  const goalY = goalTargetKg
    ? padding + ((maxWeight - goalTargetKg) / (maxWeight - minWeight)) * (height - padding * 2)
    : null;
  ...
  return (
    <View style={styles.container}>
      <Svg width="100%" viewBox={`0 0 ${width} ${height}`} height={height}>
        ...
        {goalY ? (
          <>
            <Line x1={padding} y1={goalY} x2={width - padding} y2={goalY} stroke={colors.primary} strokeDasharray="5 5" strokeWidth={1.5} />
            <SvgText x={width - padding} y={goalY - 4} fill={colors.primary} fontSize={10} textAnchor="end">
        ...
        {scaled.map((point) => (
          <Circle key={point.log.id} cx={point.x} cy={point.y} r={4} fill={colors.primary} />
        ))}
      </Svg>
      <Text style={styles.summary}>
        Latest: {points.at(-1)?.weightKg.toFixed(1)} kg · Entries: {points.length}
      </Text>
    </View>
  );
}
...
function filterLogs(logs: BodyWeightLog[], range: ChartRange) {
  ...
  return activeLogs.filter((log) => new Date(log.measuredAt).getTime() >= cutoff);
}

const styles = StyleSheet.create({
  container: { gap: spacing.sm, padding: spacing.md, borderRadius: 16, backgroundColor: colors.card },
  empty: { padding: spacing.lg, borderRadius: 16, backgroundColor: colors.card },

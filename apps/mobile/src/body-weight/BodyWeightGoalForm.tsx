...
export function BodyWeightGoalForm({ userId }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      Alert.alert('Sync pending', error instanceof Error ? error.message : 'Goal saved locally.');
    } finally {
      setSaving(false);
    }
  }
    ...
    <View style={styles.card}>
      ...
      <View style={styles.directionRow}>
        {directions.map((item) => (
          <Pressable
            key={item}
            style={[styles.directionButton, direction === item && styles.directionButtonActive]}
            onPress={() => setDirection(item)}
          >
            <Text style={[styles.directionText, direction === item && styles.directionTextActive]}>
              {item}
            </Text>
          </Pressable>
        ))}
...
const styles = StyleSheet.create({
  ...
  directionButton: { flex: 1, alignItems: 'center', padding: spacing.sm, borderRadius: 999, borderWidth: 1, borderColor: colors.border },
  directionButtonActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  directionText: { color: colors.mutedText, fontWeight: '700', textTransform: 'capitalize' },

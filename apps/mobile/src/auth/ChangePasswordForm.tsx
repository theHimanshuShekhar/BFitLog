...
export function ChangePasswordForm() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if ('error' in result && result.error) throw new Error(result.error.message ?? 'Password change failed');
      setCurrentPassword('');
      setNewPassword('');
      setStatus('success');
  ...
  return (
    <View style={styles.card}>
      ...
      <Pressable style={styles.primaryButton} onPress={() => void submit()} disabled={status === 'saving'}>
        <Text style={styles.primaryButtonText}>{status === 'saving' ? 'Saving…' : 'Change password'}</Text>
      </Pressable>
    </View>
  );
}

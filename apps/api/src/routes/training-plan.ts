...
export const trainingPlanRoutes = new Hono<{ Variables: Variables }>()
  .get('/training-plan/template', async (c) => {
    ...
    if (!template) return c.json({ error: 'Training plan template not seeded' }, 404);
  ...
  .get('/training-plan/active', async (c) => {
    ...
    if (!template) return c.json({ error: 'Active plan template missing' }, 404);
...
async function loadTemplate(templateId: string) {
  ...
  const [exerciseRows, mediaRows] = await Promise.all([
    exerciseIds.length ? db.select().from(exercises).where(inArray(exercises.id, exerciseIds)) : [],
    exerciseIds.length
      ? db
          .select()
          .from(exerciseMedia)
        ...
        return {
          ...
          exercise: exercise
            ? {
                ...
                media: (mediaByExercise.get(exercise.id) ?? []).map((media) => ({
                  id: media.id,
                  kind: media.kind,
                  url: media.url,
                })),

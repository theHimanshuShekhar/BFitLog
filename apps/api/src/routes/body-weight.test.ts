...
describe('body weight routes', () => {
  ...
  it('upserts goal and body weight logs for current user', async () => {
    ...
    const goalResponse = await app.request('/body-weight/goal', {
      ...
      body: JSON.stringify({ userId, targetKg: 85, direction: 'lose', updatedAt: now }),
    });
    ...
    const logResponse = await app.request(`/body-weight/logs/${logId}`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json', cookie },
      body: JSON.stringify({ id: logId, userId, measuredAt: now, weightKg: 90.5, createdAt: now, updatedAt: now }),
    });
    expect(logResponse.status).toBe(200);

    const logsResponse = await app.request('/body-weight/logs', { headers: { cookie } });
    expect(logsResponse.status).toBe(200);
    const logsBody = await logsResponse.json();
    expect(logsBody.logs).toHaveLength(1);
    expect(logsBody.logs[0]).toMatchObject({ id: logId, userId, weightKg: 90.5 });
  });
});
